...
    # >>> 요청 사항 반영: 언어 선택 세로 정렬 (horizontal=False)
    lang = st.radio("Select", ["en", "ko", "hi"], format_func=lambda x: {"en": "English", "ko": "한국어", "hi": "हिन्दी"}[x], horizontal=False)
    _ = LANG[lang]
...
if len(st.session_state.route) > 1:
    # >>> 요청 사항 반영: 도시간 선 색상 변경 (초록색 #90EE90 -> 빨간색 #8B0000)
    folium.PolyLine([coords[c] for c in st.session_state.route], color="#8B0000", weight=4).add_to(m)
for city in st.session_state.route:
