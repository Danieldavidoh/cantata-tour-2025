...
def render_map():
    ...
    with col_add:
        if st.session_state.admin:
            if st.button(_("add_city"), use_container_width=True, key="btn_add_city"):
    ...
    for idx, city in enumerate(cities_data):
        ...
        with st.expander(f"{city['city']} | {city.get('perf_date', '')}", expanded=expanded):
            ...
            if st.session_state.admin:
                c1, c2 = st.columns(2)
                with c1:
                    if st.button(_("edit"), key=f"edit_{idx}_{city['city']}"):
                        st.session_state.edit_city = city["city"]
                        st.rerun()
                with c2:
                    if st.button(_("remove"), key=f"del_{idx}_{city['city']}"):
                        cities_data.pop(idx)
                        save_json(CITY_FILE, cities_data)
                        st.session_state.expanded = {}
                        st.toast("도시 삭제됨")
