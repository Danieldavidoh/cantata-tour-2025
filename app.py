...
        "reset_btn": "Reset All", "venues_dates": "Tour Route", "performance_date": "Performance Date (Calendar Only)",
        "venue_name": "Venue Name", "seats": "Seats", "indoor_outdoor": "Indoor/Outdoor",
        "indoor": "Indoor", "outdoor": "Outdoor", "google_link": "Google Maps Link (for Navigation)",
        "special_notes": "Special Notes", "register": "Register Venue", "add_venue": "Add Venue",
...
        "reset_btn": "전체 초기화", "venues_dates": "투어 경로", "performance_date": "공연 날짜 (달력만 사용)",
        "venue_name": "공연장 이름", "seats": "좌석 수", "indoor_outdoor": "실내/실외",
        "indoor": "실내", "outdoor": "실외", "google_link": "구글 지도 링크 (내비게이션용)",
        "special_notes": "특이사항", "register": "공연장 등록", "add_venue": "공연장 추가",
...
        "reset_btn": "सब रीसेट करें", "venues_dates": "टूर मार्ग", "performance_date": "प्रदर्शन तिथि (केवल कैलेंडर)",
        "venue_name": "स्थल का नाम", "seats": "सीटें", "indoor_outdoor": "इंडोर/आउटडोर",
        "indoor": "इंडोर", "outdoor": "आउटडोर", "google_link": "गूगल मैप्स लिंक (नेविगेशन के लिए)",
        "special_notes": "विशेष टिप्पणियाँ", "register": "स्थल रजिस्टर करें", "add_venue": "स्थल जोड़ें",
...
    /* Expander Header Customization for Car Icon positioning (Right side text is tricky in Streamlit) */
...
            
            # ----------------------------------------------------------------------
            # Expander Title / Icon Logic (수정됨)
            # ----------------------------------------------------------------------
            nav_link = ""
            venue_count = len(t[city]) if has else 0
            if has and not t[city].empty:
                # 첫 번째 등록된 공연장의 구글 맵 링크를 사용
                first_venue_link = t[city].iloc[0].get("Google Maps Link", "")
                nav_link = nav(first_venue_link)

            # 아이콘 (자동차 모양) 설정: 링크가 있으면 🚗, 없으면 빈 문자열
            icon_in_title = f' <a href="{nav_link}" target="_blank" style="text-decoration:none;font-size:1.2em;">🚗</a>' if nav_link else ''
            
            # Expander 제목 (도시, 날짜, 공연장 수 + 아이콘)
            title_html_content = f"**{city}** – {date_str(city)} ({venue_count} {_['add_venue']}){icon_in_title}"

            with st.expander(title_html_content, expanded=False):
                
                # ----------------------------------------------------------------------
                # 1. 공연 날짜 입력 (달력만 사용)
                # ----------------------------------------------------------------------
                cur = st.session_state.dates.get(city, datetime.now().date())
                # st.date_input은 기본적으로 달력 UI를 사용하며, 별도로 키보드 입력을 막는 파라미터는 없습니다.
...
                        with col3: google_link = st.text_input(_["google_link"], placeholder="https://maps.app.goo.gl/...", key=f"l_{city}_v3")
                        with col4:
                            # Indoor/Outdoor Toggle
                            io_key = f"io_{city}_v3"
                            st.session_state.setdefault(io_key, _["outdoor"])
                            # Use st.selectbox instead of a button toggle for better Streamlit consistency
                            selected_type = st.selectbox(_["indoor_outdoor"], [_["indoor"], _["outdoor"]], key=io_key, label_visibility="collapsed")
                            
                        # 특이사항 & 등록 버튼
                        sn_col, btn_col = st.columns([4,1])
                        with sn_col: special_notes = st.text_area(_["special_notes"], key=f"sn_{city}_v3")
                        with btn_col:
                            # 버튼을 아래에 배치
                            st.markdown("<div style='height: 25px;'></div>", unsafe_allow_html=True) # 공간 확보
                            if st.form_submit_button(_["register"]):
                                if not venue_name: st.error(_["enter_venue_name"])
                                else:
                                    new_row = pd.DataFrame([{"Venue": venue_name, "Seats": seats, "IndoorOutdoor": selected_type, "Google Maps Link": google_link, "Special Notes": special_notes}])
                                    t[city] = pd.concat([t.get(city, pd.DataFrame(columns=cols)), new_row], ignore_index=True)
                                    st.success(_["venue_registered"])
                                    st.session_state.expanded_city = city # Keep expander open after registration
                                    st.rerun()
...
                                st.markdown(f"[🚗 {_[ 'navigate' ]}]({nav(row['Google Maps Link'])})", unsafe_allow_html=True)
                        with col4:
                            if st.session_state.admin or st.session_state.guest_mode:
...
                                            st.rerun()
                                # Delete button and confirmation below edit form
