...
if "show_popup" not in st.session_state: st.session_state.show_popup = True  # 초기화 추가
        ...
        if notice:
            ...
            <div id="full-screen-notice">
                <div id="full-screen-notice-content">
                    <h3>{notice['title']}</h3>
                    <div>{content}</div>
                    <button onclick="window.close_notice()" style="margin-top:20px;padding:10px;background:#ff3b3b;color:white;border:none;border-radius:8px;cursor:pointer;">닫기</button>
                </div>
            </div>
            <script>
            function close_notice() {{
                // Streamlit 상태 업데이트 + 리프레시
                fetch(`/?show_full_notice=None`, {{method: 'GET'}});
            }}
            </script>
            """, unsafe_allow_html=True)
            # 버튼으로도 닫기
            if st.button("닫기", key="close_full_notice_btn"):
