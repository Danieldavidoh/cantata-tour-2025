...
if not st.session_state.admin:
    # 투어지도
    with st.expander086("투어지도", expanded=False):
        try:
    ...
    with st.expander("공지현황", expanded=False):
        if st.session_state.notice_data:
            ...
            for notice in st.session_state.notice_data:
                ...
                notice_placeholders.append((placeholder, notice))
            
            for placeholder, notice in notice_placeholders:
                with placeholder.container():
                    col1, col2 = st.columns([9, 1])
                    with col1:
                        if st.button(notice["title"], key=f"admin_notice_view_{notice['id']}"):
                            st.session_state.show_full_notice = notice["id"]
                            st.rerun()
                    with col2:
                        if st.button("X", key=f"admin_notice_delete_{notice['id']}"):
