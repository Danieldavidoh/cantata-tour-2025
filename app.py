...
<style>
    ...
    .christmas-title .year{color:white!important;
        text-shadow:0 0 5px #FFF,0 0 10px #FFF,0 0 15px #FFF,0 0 20px #00BFFF;}
    .christmas-title::before{content:"❄️ ❄️ ❄️";position:absolute;top:-20px;left:50%;
        transform:translateX(-50%);font-size:0.6em;color:white;
        animation:snow-fall 3s infinite ease-in-out;}
    @keyframes snow-fall{0%,100%{transform:translateX(-50%) translateY(0);}
        50%{transform:translateX(-50%) translateY(10px);}}
    h1,h2,h3{color:#90EE90;text-shadow:1px 1px 3px #8B0000;text-align:center;}
    .stButton>button{background:#228B22;color:white;border:2px solid #8B0000;
    ...
    .stMetric{background:rgba(34,139,34,0.3);border:2px solid #90EE90;
        border-radius:12px;padding:10px;}
    .stExpander{background:rgba(139,0,0,0.4);border:1px solid #90EE90;
        border-radius:12px;}
    ...
    .christmas-decoration{position:absolute;font-size:2.5em;pointer-events:none;
        animation:float 6s infinite ease-in-out;z-index:10;}
    ...
    .snowflake{position:absolute;color:rgba(255,255,255,0.9);font-size:1.2em;
        pointer-events:none;animation:fall linear infinite;opacity:0.9;}
    @keyframes fall{0%{transform:translateY(-100vh) rotate(0deg);opacity:0.9;}
        100%{transform:translateY(100vh) rotate(360deg);opacity:0;}}
    .city-link{color:#90EE90;text-decoration:underline;cursor:pointer;
...
for i in range(80):
    ...
    snow += f'<div class="snowflake" style="left:{left}%;font-size:{size};animation-duration:{dur}s;animation-delay:{delay}s;">❄️</div>'
...
with st.sidebar:
    ...
    # 세션 초기화
    for key in ["admin", "show_pw", "guest_mode"]:
...
with left_col:
    ...
    if st.session_state.route:
        ...
        for city in st.session_state.route:
            ...
            # ----- venue 존재 여부 -----
            has_venue = city in target and not target[city].empty

            # ----- expander 라벨 -----
            icon = ""
            if has_venue:
                first_link = target[city].iloc[0]["Google Maps Link"]
                if first_link and first_link.startswith("http"):
                    nav = f"https://www.google.com/maps/dir/?api=1&destination={first_link}&travelmode=driving"
                    icon = f' [🗺️]({nav})'
            expander_label = f"**{city}** – {date_str} ({len(target[city]) if has_venue else 0} venues){icon}"

            with st.expander(expander_label, expanded=False):
                # ----- 날짜 입력 -----
                cur_date = st.session_state.dates.get(city, datetime.now().date())
                new_date = st.date_input(_["performance_date"], cur_date, key=f"date_{city}")
                if new_date != cur_date:
                    st.session_state.dates[city] = new_date
                    st.success(_["date_changed"])
                    st.rerun()

                # ----- venue 입력 폼 (admin/guest만) -----
                        ...
                        with st.container():
                            ...
                            with c4:
                                ...
                                if st.button(f"**{st.session_state[io_key]}**", key=f"io_toggle_{city}"):
                                    st.session_state[io_key] = (
                                        _["indoor"]
                                        if st.session_state[io_key] == _["outdoor"]
                                        else _["outdoor"]
                                    )
                                    st.rerun()
                                ...
                                if st.button(f"**{_['register']}**", key=f"register_{city}"):
                                    if not venue_name:
                                        ...
                                        if city not in target:
                                            target[city] = pd.DataFrame(columns=cols)
                    ...
                    for idx, row in target[city].iterrows():
                        ...
                        with c3:
                            if row["Google Maps Link"].startswith("http"):
                                nav = f"https://www.google.com/maps/dir/?api=1&destination={row['Google Maps Link']}&travelmode=driving"
                                st.markdown(f"[🗺️ {_['navigate']}]({nav})", unsafe_allow_html=True)
                        with c4:
                            if st.session_state.admin or st.session_state.guest_mode:
                                edit_key = f"edit_{city}_{idx}"
                                if st.button(_["edit_venue"], key=f"edit_btn_{city}_{idx}"):
                                    st.session_state[edit_key] = True
                                if st.button(_["delete_venue"], key=f"del_btn_{city}_{idx}"):
...
with right_col:
    ...
    for city in st.session_state.route:
        ...
        date_obj = st.session_state.dates.get(city)
        date_str = date_obj.strftime(_["date_format"]) if date_obj else "TBD"
        popup_html = f"<b style='color:#8B0000'>{city}</b><br>{date_str}"
        if link:
            nav = f"https://www.google.com/maps/dir/?api=1&destination={link}&travelmode=driving"
            popup_html = f'<a href="{nav}" target="_blank" style="color:#90EE90">{popup_html}<br><i>{_["navigate"]}</i></a>'
        folium.CircleMarker(
