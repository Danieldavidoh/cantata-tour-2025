...
# --- 눈송이 생성: 26개 (2/3 감소) ---
for i in range(26):  # 80 → 26
    left = random.randint(0, 100)
    duration = random.randint(10, 20)  # 더 느리게
    size = random.uniform(0.8, 1.4)   # 더 작게
...
def add_notice(title, content, img=None, file=None):
    ...
    st.session_state.new_notice = True; st.session_state.alert_active = True; st.session_state.current_alert_id = notice["id"]
    play_carol(); st.rerun()

def format_notice_date(d):
    try:
        dt = datetime.strptime(d, "%m/%d %H:%M"); today = date.today()
        if dt.date() == today: return f"{_(f'today')} {dt.strftime('%H:%M')}"
        elif dt.date() == today - timedelta(days=1): return f"{_(f'yesterday')} {dt.strftime('%H:%M')}"
        else: return d
    except: return d
...
def render_notices():
    ...
    for i, n in enumerate(data):
        title = f"{format_notice_date(n['date'])} | {n['title']}"
        ...
        with st.expander(title, expanded=expanded):
            ...
            if not st.session_state.admin and n["id"] not in st.session_state.seen_notices and expanded:
                ...
                if n["id"] == st.session_state.current_alert_id: st.session_state.alert_active = False
                st.rerun()
            if expanded and exp_key not in st.session_state.expanded_notices: st.session_state.expanded_notices.append(exp_key)
            elif not expanded and exp_key in st.session_state.expanded_notices: st.session_state.expanded_notices.remove(exp_key)

    if not st.session_state.admin and st.session_state.alert_active and st.session_state.current_alert_id:
        play_carol()
        st.markdown(f'<div class="alert-box" id="alert"><span>{_("new_notice_alert")}</span><span class="alert-close" onclick="document.getElementById(\'alert\').remove()">X</span></div>', unsafe_allow_html=True)
...
def render_map():
    ...
    for i, c in enumerate(cities):
        is_past = c.get('perf_date') and c['perf_date'] != "9999-12-31" and datetime.strptime(c['perf_date'], "%Y-%m-%d").date() < date.today()
        ...
        popup_html = f"<div style='font-size:14px;line-height:1.6;'><b>{c['city']}</b><br>날짜: <b>{perf_date_formatted}</b><br>장소: {c.get('venue','—')}<br>인원: {c.get('seats','—')}<br>유형: {indoor_text}</div>"
        ...
        if i < len(cities) - 1:
            ...
            AntPath([coords, nxt_coords], color="#e74c3c", weight=6, opacity=0.3 if is_past else 1.0, delay=800, dash_array=[20, 30]).add_to(m)

        with st.expander(f"{c['city']} | {format_date_with_weekday(c.get('perf_date'))}"):
            st.markdown(f"<div><span style='font-weight:bold;color:#e74c3c;'>장소:</span> {c.get('venue','—')}</div>", unsafe_allow_html=True)
            st.markdown(f"[길 안내]({google_nav})", unsafe_allow_html=True)

            if st.session_state.admin:
                c1, c2 = st.columns(2)
                with c1: st.button("수정", key=f"edit_{c['city']}")
                with c2: st.button("삭제", key=f"del_{c['city']}")
...
if st.session_state.tab_selection == _(f"tab_notice"):
    if st.session_state.admin:
        with st.form("notice_form", clear_on_submit=True):
            ...
            img = st.file_uploader("이미지", type=["png", "jpg", "jpeg"]); file = st.file_uploader("첨부 파일")
