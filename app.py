...
def import_cities_from_csv():
    ...
    current_city_names = {c['city'] for c in current_cities}

    added = 0
    for city_name in new_cities:
        if city_name not in current_city_names:
            # 기본값으로 추가 (좌표는 Pune 중심)
            new_city = {
                "city": city_name,
                "venue": "",
                "seats": "",
            ...
            current_city_names.add(city_name)
            added += 1
