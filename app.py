...
        "indoor": "Indoor", # 언어 중립적인 키로 사용
        "outdoor": "Outdoor", # 언어 중립적인 키로 사용
...
        "submit": "Submit",
        "io_indoor_display": "Indoor", # 화면 표시에 사용
        "io_outdoor_display": "Outdoor", # 화면 표시에 사용
    },
    "ko": {
...
        "submit": "제출",
        "io_indoor_display": "실내", # 화면 표시에 사용
        "io_outdoor_display": "실외", # 화면 표시에 사용
    },
    "hi": {
...
        "submit": "जमा करें",
        "io_indoor_display": "इंडोर", # 화면 표시에 사용
        "io_outdoor_display": "आउटडोर", # 화면 표시에 사용
    },
}
...
            # *수정: 한국어 '투어 시작' 대신 다국어 지원이 되는 메세지로 변경*
...
                    # *수정: 날짜 계산 시 timedelta에 days=1을 추가하여 최소한 하루가 지나도록 조정*
                    st.session_state.dates[new_city] = (datetime.combine(prev_date, datetime.min.time()) + timedelta(hours=hrs, days=1)).date()
                # *수정: 한국어 '추가됨' 대신 다국어 지원이 되는 메세지로 변경*
                st.success(f"{new_city} {_['add_btn'].replace('➕ ', '')}")
                st.rerun()
        with col_next:
...
    st.subheader(_["venues_dates"])
    ...
    # 헬퍼 함수: 언어 중립적인 키를 현재 언어의 표시 문자열로 변환
    def get_io_display(io_key, _):
        if io_key == 'Indoor':
            return _['io_indoor_display']
        elif io_key == 'Outdoor':
            return _['io_outdoor_display']
        return io_key
...
                # *수정: 한국어 '날짜 변경됨' 대신 다국어 지원이 되는 메세지로 변경*
                st.success(f"{_['performance_date']} {_['save']}")
                st.rerun()
...
                for idx, row in df.iterrows():
                    # *수정: 저장된 언어 중립 키(Indoor/Outdoor)를 현재 언어로 표시*
                    io_display = get_io_display(row['IndoorOutdoor'], _)
...
                    with c3:
                        # *수정: 세션 상태에 언어 중립적인 키(Indoor/Outdoor)를 저장하고 표시할 때만 현재 언어를 사용*
                        io_key = st.session_state.get(f"io_{city}", 'Outdoor') # 기본값을 언어 중립적인 'Outdoor'로 설정
                        if st.button(_["indoor_outdoor"], key=f"io_btn_{city}", use_container_width=True): # use_container_width 추가
                            io_key = 'Indoor' if io_key == 'Outdoor' else 'Outdoor' # 언어 중립적인 키를 토글
                            st.session_state[f"io_{city}"] = io_key
                        st.markdown(f"**{get_io_display(io_key, _)}**") # 현재 언어로 표시
...
                        # *수정: 한국어 '등록 완료' 대신 다국어 지원이 되는 메세지로 변경*
                        st.success(f"{_['register']} {_['save']}")
                        st.rerun()
...
EOF && \
git add app.py && \
git commit -m "fix: Multilingual UI logic error in indoor/outdoor toggle and improved i18n messages" && \
git push
