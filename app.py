...
def render_map():
    ...
    for i, c in enumerate(cities):
        ...
        try:
            ...
            perf_date_obj = None
        # 마커 선택: 오늘+미래 = 구글맵 스타일, 과거 = 흐리게 (opacity=0.5)
        ...
        icon_html = google_balloon.format(color=color, inner_color=inner_color)
        icon_html = icon_html.replace('<div style="position: relative', f'<div style="position: relative; {opacity_style}')
            ...
            if st.session_state.admin and not st.session_state.get("edit_city"):
                ...
                with c2:
                    if st.button("삭제", key=f"del_{i}"):
                        ...
                        st.rerun()
        # 도시간 거리 및 시간 표시 완벽 삭제 - 총 거리만 계산
        if i < len(cities)-1:
            next_c = cities[i+1]
            dist_km, _ = get_real_travel_time(c['lat'], c['lon'], next_c['lat'], next_c['lon'])
            total_dist += dist_km
...
with tab1:
    ...
    render_notices()
with tab2:
    render_map() 에서 지도위 연결 라인위에 라인과 평행하게 라인위에 도시간의 거리와 시간 표시 . 항상 라인 위쪽에 배치. 도시표시 아이콘은 일반 빨간 풍선, 모양 날짜가 지난 도시는 투명한풍선, 오늘 날짜 도시는 지금의 구글맵 스타일 풍선으로 해줘
