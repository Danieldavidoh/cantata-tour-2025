        ...
        with st.expander(city, expanded=False):
            ...
            if not df.empty:
                for idx, row in df.iterrows():
                    # 저장된 언어 중립 키(Indoor/Outdoor)를 현재 언어로 표시
            ...
            if st.session_state.admin or city not in st.session_state.admin_venues:
                with st.form(key=f"add_{city}"):
                    ...
                    with c3:
                        # 세션 상태에 언어 중립적인 키(Indoor/Outdoor)를 저장
                        io_key = st.session_state.get(f"io_{city}", 'Outdoor')
                        if st.button(_["indoor_outdoor"], key=f"io_btn_{city}", use_container_width=True):
...
CODE_END
git add app.py
git commit -m "Definitive fix: Erased all shell commands from app.py content and confirmed pure Python"
git push
