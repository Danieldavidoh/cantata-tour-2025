...
import math
import random
from geopy.distance import great_circle # 두 지점 간의 직선 거리 계산 (실제 도로 거리가 아님)
...
LANG = {
    "en": {
        ...
        "guest_view_mode": "Guest View Mode"
    },
    "ko": {
    ...
    "hi": {
        ...
        "guest_view_mode": "गेस्ट व्यू मोड"
    },
}
...
# 5. 도시 및 좌표 데이터 (마하라스트라 주 도시 약 200개 요청에 따라 주요 도시 목록 유지)
cities = sorted([
...
def date_str(city): 
    ...
    return d.strftime(_["date_format"]) if d and isinstance(d, datetime.date) else _["no_performance"]

def nav_url(link): 
    return f"https://www.google.com/maps/search/?api=1&query={link}" if link and link.startswith("http") else "#" # 네비게이션 링크

def calculate_distance_time(city1, city2):
    # ⚠️ 실제 API 호출 대신 임의의 값으로 대체 (요구사항 충족을 위한 임시 처리)
    if city1 in coords and city2 in coords:
        # 직선 거리 계산 (실제 도로 거리/시간 아님)
...
if st.session_state.admin:
    ...
    df_route = target_df() # 현재 사용하는 데이터프레임 딕셔너리 (admin_venues 또는 venues)
    ...
    with left_col:
        ...
        with col_sel:
            # 도시 선택 (기본값: "공연 없음")
            current_city_selection = st.session_state.route[0] if st.session_state.route else _["city_placeholder"]
            if current_city_selection not in avail and current_city_selection not in st.session_state.route:
                 current_city_selection = _["city_placeholder"]
                 
            city_options = [_["city_placeholder"]] + avail
            selected_city_name = st.selectbox(_["select_city"], city_options, key="city_select_admin", index=city_options.index(current_city_selection) if current_city_selection in city_options else 0)
        ...
        with col_btn:
            if st.button(_["add_city_to_route"], key="add_route_city_admin") and selected_city_name != _["city_placeholder"]:
                if selected_city_name not in st.session_state.route:
                    ...
                    st.session_state.dates[selected_city_name] = datetime.now().date() # 기본 날짜 설정
        ...
        if st.session_state.route:
            ...
            for i, city in enumerate(st.session_state.route):
                ...
                with st.expander(expander_label, expanded=False):
                    ...
                    if current_io == _["outdoor"]:
                        btn_color_class = "io_btn_out"
                        btn_text = f"{_['outdoor']} / {_['indoor']}"
                    else:
                        btn_color_class = "io_btn_in"
                        btn_text = f"{_['indoor']} / {_['outdoor']}"
                        
                    col_io, col_reg = st.columns([1, 1])
                    with col_io:
                        if st.button(btn_text, key=f"io_btn_{city}", help="실외/실내 전환", help="Toggle Indoor/Outdoor", 
                                     use_container_width=True, help="", 
                                     **{'class': btn_color_class}):
                ...
                elif i == 0 and city_coord:
                    total_dist += 0.0 # 시작점은 0
                    total_time_min += 0
    ...
    with right_col:
        ...
        for i, city in enumerate(st.session_state.route):
            if city in coords:
                ...
                city_markers[city] = coords[city]

        # 경로 선 연결 (빨간색 점선 화살표)
        if len(path_coords) > 1:
            # 거리 및 소요 시간 표시 로직은 API 부재로 지도 위에는 구현이 복잡하여 경로 상자에 표시했습니다.
...
# ------------------- 게스트 모드 (보기 모드) -------------------
else: # 게스트 모드 (st.session_state.guest_mode == True 이거나 admin == False & show_pw == False)
    st.subheader(_["guest_view_mode"])
    
    # 게스트 모드에서도 언어 선택은 사이드바에서 이미 되었으므로 _는 유효함
    ...
    with left_col:
        ...
        if st.session_state.route:
            ...
            # 총 거리/시간 계산 및 표시 (게스트 모드에서도 계산)
            total_dist = 0.0
            total_time_min = 0
            for i in range(1, len(st.session_state.route)):
                city1 = st.session_state.route[i-1]
                city2 = st.session_state.route[i]
                if city1 in coords and city2 in coords:
                    dist_str_calc, time_str_calc = calculate_distance_time(city1, city2)
                    total_dist += float(dist_str_calc.split(' ')[0])
                    total_time_min += int(time_str_calc.split(' ')[0])
