# app.py - 칸타타 투어 2025 (최종 완전판 + 라인과 완벽 수평 라벨 + 마커 조건 수정) 🔥
# 라벨: 라인과 **정확히 수평** + **라인 위쪽** / 마커: 오늘+미래 = 구글맵 스타일, 과거 = 투명
...
def render_map():
    ...
    <div style="position: relative; width: 30px; height: 40px; margin-left: -15px; margin-top: -40px;">
        ...
        <div style="position: absolute; top: 9px; left: 9px; width: 12px; height: 12px; background: {inner}; border-radius: 50%; transform: rotate(45deg);"></div>
    </div>
    '''

    # 투명 풍선 (과거)
    transparent_balloon = '''
    <div style="position: relative; width: 30px; height: 40px; margin-left: -15px; margin-top: -40px; opacity: 0.4;">
        <div style="position: absolute; bottom: 0; left: 0; width: 30px; height: 30px; background: #cccccc; border-radius: 50% 50% 50% 0; transform: rotate(-45deg); box-shadow: 0 0 6px rgba(0,0,0,0.3);"></div>
        <div style="position: absolute; top: 6px; left: 6px; width: 18px; height: 18px; background: white; border-radius: 50%; transform: rotate(45deg);"></div>
        <div style="position: absolute; top: 9px; left: 9px; width: 12px; height: 12px; background: #999999; border-radius: 50%; transform: rotate(45deg);"></div>
    </div>
    '''
    ...
    for i, c in enumerate(cities):
        ...
        # 마커 선택: 오늘+미래 = 구글맵 스타일, 과거 = 투명
        if perf_date_obj and perf_date_obj < today:
            icon_html = transparent_balloon
        else:
            # 오늘 또는 미래: 실내/야외에 따라 색상
            color = "#ea4335" if c.get("indoor") else "#4285f4"
            icon_html = google_balloon.format(color=color, inner="#ffffff")
        ...
        if i < len(cities)-1:
            ...
            # 라인 위쪽으로 오프셋
            offset_dist = 0.0008
            offset_lat = offset_dist * cos(radians(bearing + 90))
            offset_lon = offset_dist * sin(radians(bearing + 90)) / cos(radians(mid_lat))

            label_lat = mid_lat + offset_lat
